...
# ==============================================================================
def setup_logging(log_dir='gaussian/logs', experiment_name='smr_seld_gaussian'):
...
class Config:
    ...
    # All outputs go under "gaussian" folder for easy identification
    GAUSSIAN_BASE = BASE_PATH / "gaussian"
    OUTPUT_PATH = GAUSSIAN_BASE / "outputs"
    CHECKPOINT_PATH = GAUSSIAN_BASE / "checkpoints"
    ...
    def __init__(self):
        # Create gaussian base directory and subdirectories
        self.GAUSSIAN_BASE.mkdir(exist_ok=True, parents=True)
...
class SELDDataset(Dataset):
    def __init__(self, audio_files, metadata_files, num_classes=14, use_gaussian_augmentation=False):
        """
        ...
        self.use_gaussian_augmentation = use_gaussian_augmentation
        ...
        logger.info(f"  Label augmentation: {'Gaussian noise' if self.use_gaussian_augmentation else 'Standard (metadata_to_labels)'}")
    ...
    def _load_and_concatenate_all(self):
        ...
        for idx, (audio_path, metadata_path) in enumerate(tqdm(
            ...
            try:
                ...
                # Use Gaussian augmentation for training, standard labels for testing
                if self.use_gaussian_augmentation:
                    labels, _, _ = augment_with_gaussian_noise(
                        metadata_path,
                        audio_duration,
                        sample_rate=sr,
                        I=self.I,
...
class SMRSELDWithCSPDarkNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Softmax over classes – losses expect probabilities
...
def train_model(
    ...
    logger.info("\nStep 1: Creating data loaders from provided datasets...")
    
    train_loader = DataLoader(
        train_dataset,
        batch_size=batch_size,
        shuffle=True,
        num_workers=4 if torch.cuda.is_available() else 0,  # Multi-worker for HPC
...
def test_model(
    ...
    logger.info("\nStep 1: Creating test data loader from provided dataset...")
...
def main():
    ...
    logger.info("="*80)
    logger.info("SMR-SELD TRAINING WITH GAUSSIAN AUGMENTATION")
    logger.info("="*80)
    logger.info(f"Output directory: {config.GAUSSIAN_BASE}")
    logger.info(f"  - Logs: gaussian/logs/")
    logger.info(f"  - Checkpoints: {config.CHECKPOINT_PATH}")
    logger.info(f"  - Outputs: {config.OUTPUT_PATH}")
    logger.info(f"Log file: {log_file}")
    logger.info(f"\nConfiguration:")
    logger.info(f"  Use full dataset: {config.USE_FULL_DATASET}")
    logger.info(f"  Gaussian augmentation: ENABLED (training only)")
    ...
    try:
        ...
        logger.info("\nCreating datasets...")
        logger.info("  - Training dataset (with Gaussian augmentation)...")
        train_dataset = SELDDataset(
            audio_files=train_audio_files,
            metadata_files=train_meta_files,
            num_classes=config.NUM_CLASSES,
            use_gaussian_augmentation=True  # Use Gaussian noise augmentation for training
        )
        
        logger.info("  - Test dataset (standard labels)...")
        test_dataset = SELDDataset(
            audio_files=test_audio_files,
            metadata_files=test_meta_files,
            num_classes=config.NUM_CLASSES,
            use_gaussian_augmentation=False  # Use standard labels for testing
        )
        
        logger.info(f"Datasets created: {len(train_dataset)} train windows, {len(test_dataset)} test windows")
        
        # Train model (pass datasets directly)
